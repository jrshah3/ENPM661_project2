...
def run_backward_bfs(
    start: tuple[int, int],
    goal: tuple[int, int],
    blocked_mask: np.ndarray,
):
    open_queue = deque([goal])

    visited = {goal}
    parent = {}
    explored_order = []
    steps_to_node = {goal: 0}
    ...
    while open_queue:
        ...
        if reached_start(current_node, start):
            ...
            return {
                ...
                "total_cost": steps_to_node[start],
            }

        for _, neighbor, _ in get_valid_neighbors(current_node, blocked_mask):
...
def main():
    ...
    print("\n===== RUNNING BACKWARD BFS =====")
    print("Search starts from GOAL and stops when START is dequeued.")

    t0 = time.time()
    result = run_backward_bfs(start, goal, blocked_mask)
    t1 = time.time()
